...
function RoundPage() {
  ...
  const [error, setError] = useState<string | null>(null);
  const load = async () => { if (!roundId) return; try { setError(null); setRound(await contractAdapter.getRound(roundId)); } catch (cause) { setError(cause instanceof ContractNotConfiguredError ? "This round will be readable after the Studionet contract is configured." : "The round could not be loaded."); } };
  useEffect(() => { void load(); }, [roundId]);
